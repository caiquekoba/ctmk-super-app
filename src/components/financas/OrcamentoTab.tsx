...
export default function OrcamentoTab({ month }: OrcamentoTabProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Monta lista (apenas categorias com gasto OU orçamento definido)
      const result = expenses
        ...
        .map(e => ({
          ...
          expense_expected_amount: e.expense_expected_amount,
        ...
        <div className="card-glow">
          ...
          <div className="flex justify-between mt-1.5">
            ...
            <span className={cn('text-xs font-medium',
              totalSpent > totalBudget ? 'text-rose-400' : 'text-slate-400'
            )}>
      ...
      {overBudget.length > 0 && (
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-rose-400/10 border border-rose-400/20">
          ...
          <p className="text-rose-300 text-sm">
            <strong>{overBudget.length}</strong> {overBudget.length === 1 ? 'categoria estourou' : 'categorias estouraram'} o orçamento este mês.
          </p>
        </div>
      )}
                  ...
                  <div className="flex items-center justify-between mt-0.5">
                    ...
                    {hasBudget && (
                      <span className="text-xs text-slate-500">
                        de {formatBRL(cat.expense_expected_amount!)}
                      </span>
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex justify-between mt-1">
                    ...
                    {isOver && (
                      <span className="text-xs text-rose-400 font-medium">
                        + {formatBRL(cat.total_spent - (cat.expense_expected_amount ?? 0))} acima
                      </span>
                    )}
                    {!isOver && hasBudget && (
                      <span className="text-xs text-slate-600">
                        Resta {formatBRL((cat.expense_expected_amount ?? 0) - cat.total_spent)}
                      </span>
                    )}
                  </div>
                </>
